...
class CoordinationManager:
    ...
    def _ensure_coordination_dir(self):
        ...
        try:
            ...
            raise CoordinationFileError(self.coordination_dir, "create directory", e)
    ...
    def read_controller_state(self) -> Optional[CoordinationState]:
        ...
        try:
            ...
            if not state.is_valid():
                logger.warning(
                    f"Found stale controller state (age: {time.time() - state.timestamp}s)"
                )
                return None
    ...
    def wait_for_controller(
        ...
        while time.time() - start_time < timeout:
            ...
            # Exponential backoff
            check_interval = min(
                check_interval * self.config.backoff_factor, self.config.max_interval
            )
