...
class XdistConfig:
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        config = {
            "enabled": self._get_bool_env(
                self.ENABLE_CONTROLLER_PRECREATION, 
                default=False
            ),
            "debug": self._get_bool_env(
                self.ENABLE_COORDINATION_DEBUG,
                default=False
            ),
            "worker_timeout": self._get_int_env(
                "TESTMON_XDIST_WORKER_TIMEOUT",
                default=self.DEFAULT_WORKER_TIMEOUT
            ),
    ...
    def get_ready_marker_path(self, rootdir: str) -> str:
        ...
        return os.path.join(
            self.get_coordination_dir(rootdir),
